...
module PathFinding.Algorithm:DepthFirstSearch;
...
bool DepthFirstSearch1(std::shared_ptr<StaticMap>& staticMap, const Vec2D<i32>& startPos, const Vec2D<i32>& destPos, std::vector<Vec2D<i32>>* outResult)
{
    ...
    while (tls_PathFindingNodePQ.Size() > 0)
    {
        ...
        //
        std::vector<PathFindingNode*> nextNodeStack;
        ...
        for (auto [toAdjNode, toDir] : staticMap->GetAdjacentNodesAt(processingNode->x, processingNode->y))
        {
            ...
            nextNodeStack.push_back(nextNode);
        }

        // 들어간 것을 뒤집어서 조회한다.
        while (nextNodeStack.size() > 0)
        {
            PathFindingNode* nextNode = nextNodeStack.back();
            nextNodeStack.pop_back();
    
            // 마지막에 들어간 노드의 비용을 가장 저렴하게 만들기 위한 방법
...
bool DepthFirstSearchAdvance(std::shared_ptr<PathFindingContext>& pathFindingContext)
{
    ...
    if (pathFindingContext->GetAlgorithmType() != AlgorithmType::DepthFirst)
        return false;
    ...
    switch (phase)
    {
        ...
        case PathFindingContext::Phase::PathFinding:
        {
            ...
            // TEMP
            // std::cout << "*" << pathFindingContext->GetOpenSetEnqueuedCount() << " : " << processingNode->f << '\n';
            ...
            // 대각선 O : SouthWest -> SouthEast -> NorthEast -> NorthWest -> West -> South -> East -> North
            // 대각선 X : West -> South -> East -> North
            // 위 순서대로 들어간 것을 뒤집어서 조회한다.
            while (nextNodeStack.size() > 0)
            {
                PathFindingNode* nextNode = nextNodeStack.back();
                nextNodeStack.pop_back();
...
bool DepthFirstSearchComplete(std::shared_ptr<PathFindingContext>& pathFindingContext)
{
    ...
    while (true)
    {
        ...
        if (DepthFirstSearchAdvance(pathFindingContext) == false)
            return false;
    }
