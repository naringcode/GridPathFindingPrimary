...
export namespace PathFinding
{
     ...
     *  - 대각선 방향으로 탐색하는 과정에서 생성되며 목표 위치로 향하는 Jump Point임.
     *   - 작업 위치 기준으로 목표 위치를 가로 혹은 세로으로 측정해서 보았을 때
     *     그 거리가 벽 또는 다음 점프 지점까지의 거리보다 짧거나 같으면 생성됨.
     *  - 쉽게 생각하면 런타임 도중 직접 환승 지점(Target Jump Point)를 만들어 목적지까지 직행하는 것을 시도하는 것이라 보면 됨.
     * 
     * Target Jump Point가 생성될 조건을 충족했으면 다음 순서에 따라 Jump Point를 생성하면 된다.
     *  1. 목표 지점까지의 가로 거리와 세로 거리 중 최솟값을 취함.
     *  2. 그 크기만큼 대각선 방향으로 이동하는 Jump Point를 생성함.
     * 
     * 만약 대각선 탐색 방향에 목표 지점이 있다면?
     *  - 새로운 Target Jump Point의 위치는 목표 노드의 위치와 동일할 것임.
     * 
     * 설명은 거창하지만 실제 구동 코드를 보면 쉽게 이해할 수 있다.
     * 맵을 Baking 하는 과정과 이를 통해 탐색하는 과정 자체는 별개의 것임을 구분해서 봐야 한다.
     */
